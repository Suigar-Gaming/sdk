...
- exposing `packageIds.legacyNft` and `objectIds.legacyNftFactory` for applications that need legacy NFT lookups without adding an NFT client surface to this SDK; derive `${client.suigar.getConfig().packageIds.legacyNft}::nft::Nft`, call `client.core.listOwnedObjects({ owner, type: nftType, include: { content: true } })`, and decode results with `client.suigar.bcs.LegacyNft.parse(object.content)`. Fetch catalog reads with `include: { content: true }` and decode them with `client.suigar.bcs.LegacyNftFactory.parse(object.content)`, while leaving legacy NFT minting outside this SDK
