...
- exposing `packageIds.nftV1` and `objectIds.nftV1Factory` for applications that need NFT V1 lookups without adding an NFT client surface to this SDK; derive `${client.suigar.getConfig().packageIds.nftV1}::nft::Nft`, call `client.core.listOwnedObjects({ owner, type: nftType, include: { content: true } })`, and decode results with `client.suigar.bcs.NftV1.parse(object.content)`. Fetch catalog reads with `include: { content: true }` and decode them with `client.suigar.bcs.NftV1Factory.parse(object.content)`, while leaving NFT V1 minting outside this SDK
