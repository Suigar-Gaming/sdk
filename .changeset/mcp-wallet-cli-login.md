...
Make local browser origins, bridge timeout, and callback body-size limits configurable with `SUIGAR_MCP_WEB_URL`, `SUIGAR_MCP_BRIDGE_TIMEOUT_MS`, `SUIGAR_MCP_BRIDGE_MAX_BODY_BYTES`, `--web-url`, `--bridge-timeout-ms`, and `--bridge-max-body-bytes`, and have bridge creation open its URL by default unless disabled.
