...
- **Command-line wallet management**
  ...
  - Set `SUIGAR_MCP_BRIDGE_TIMEOUT_MS` or pass `--bridge-timeout-ms` to control how long login, logout, and approval bridges wait before expiring. The default is `300000`.
  - Set `SUIGAR_MCP_BRIDGE_MAX_BODY_BYTES` or pass `--bridge-max-body-bytes` to control the maximum browser callback JSON body size. The default is `16384`.
