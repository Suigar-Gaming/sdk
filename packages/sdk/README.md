...
`packageIds.legacyNft` exposes the network-specific legacy NFT package id, so applications can derive the `::nft::Nft` type for their own lookup. The `packageIds.legacyNftFactory` object id is available for applications that need to read the legacy NFT catalog. The SDK does not provide a dedicated NFT client API or legacy NFT mint transaction builder.
