...
Public packages are published to both npm and JSR. Changesets updates the npm `package.json` versions first, then `pnpm run codegen:version` syncs the matching `jsr.json` files before the release PR is opened. `pnpm run release` builds the publishable packages, publishes them to npm through Changesets, then runs JSR publishing for each public package.
...
pnpm run publish:jsr:dry-run
pnpm run release
```

The versioning step keeps package-local `jsr.json` versions aligned with the npm package versions produced by Changesets. Run the JSR dry-run after building package output when you need to validate JSR packaging without publishing. The dry-run permits local uncommitted changes; the real publish script does not. JSR publishes the TypeScript source entrypoints; `@suigar/mcp` also includes the built MCP App HTML asset generated by the package build. The JSR publish scripts currently use `--allow-slow-types` because generated SDK source exports include inferred types that JSR flags during slow-type validation.
